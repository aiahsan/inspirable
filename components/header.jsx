...
const TopBar = ({ hastop, title, titleSpan, subtitle, hassubtitle }) => {
    if (hastop) {
        return <>
            ...
            {
                hassubtitle == true ? <p className="roboto-reg-16 text-center mt-5" style={{ maxWidth: 747, margin: 'auto' }}>
                    {subtitle}
        ...
        return <>
            {
                hassubtitle == true ? <p className="roboto-reg-16 cst-para-stl">
                    {subtitle}
                </p> : <div className="mt-5 pt-5"></div>
            }
...
export default ({ title, titleSpan, subtitle, hassubtitle, hastop,active }) => {
    const user=useSelector(x=>x.User)
    const dispatch=useDispatch();
    const history=useRouter();

    return <div className="back-header mb-5">
        <div className="ml-5 mr-5 masd02-34asdsa3" style={{ paddingBottom: '5%' }}>
            <Navbar collapseOnSelect expand="lg" >
                <Navbar.Brand as={Link} href="/" className="cst-nav-link nav-link"><img src="/images/logo.png" className="logo-main" /></Navbar.Brand>
                ...
                <Navbar.Collapse id="responsive-navbar-nav">
                    <Nav className="mr-le">
                        ...
                        <a as={Link} href="/contact" className={`cst-nav-link roboto-reg-16 nav-link ${active==4?'active-2':""}`} >Contact</a>
            ...
            <TopBar hastop={hastop} title={title} titleSpan={titleSpan} subtitle={subtitle} hassubtitle={hassubtitle} />
        </div>
    </div>
}