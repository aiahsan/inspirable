...
const BlogCard = ({ img, subtitle, title, anchor,x ,switch1}) => {
   ...
   return <div  onClick={()=>navigation.push("/blog/post/"+x.id,{cursor:'pointer'})} className="bl-card-123hisa" style={{borderRadius:'0px!important'}}>
        <img src={img} className="w-100" style={{ backgroundColor: 'F2F9FB', borderTopLeftRadius: 10, borderTopRightRadius: 10,height:200 }} />
        <div className="p-3 dsp-flex-23" style={{backgroundColor:switch1==true?"#6b6b6b":""}} >
            <p className="roboto-reg-12" style={{color:switch1==true?"white":""}}>{subtitle}</p>
            <h3 className="p-0 roboto-reg-18 text-center asdoas-32-s0af3bud8cu9jw342"style={{color:switch1==true?"white":""}} >{title}</h3>
            <p className="roboto-reg-14 mt-2" style={{color:switch1==true?"white":""}}>{anchor}</p>
            
           
        </div>
    </div>
}
const CardDSP1 = ({ image, head, title, post,x ,switch1}) => {
    const navigation=useRouter();
     ...
     <FiArrowRight/>
         ...
         <div class="card-body" style={{backgroundColor:switch1==true?"#6b6b6b":"white"}}>
             <p class="roboto-reg-14 text-center" style={{color:switch1==true?"white":""}}>{title}</p>
             <p class="roboto-reg-18 pl-2 pr-2  text-center asdoas-32-s0af3bud8cu9jw342" style={{color:switch1==true?"white":"",fontSize: 22}}  >{head}</p>
             <p class="roboto-reg-14 text-center"  style={{color:switch1==true?"white":"",textDecoration: 'none'}} ><u style={{ textDecoration: 'none' }}>{post}</u></p>
         </div>
     </div>
 }
export default () => {
    ...
    />
        ...
        <div className="container-fluid   mt-3" >
            <div className="container pt-5 pb-5">
                                ...
                                <div className="row bl-card-123hisa" onClick={()=>navigation.push("/blog/post/"+firstBlog)}    style={{backgroundColor:switch1==true?"#212529":"",borderRadius:'0px!important'}}>
     
                    <div className="col-lg-5 col-md-6 col-sm-12 mb-1 mt-5 mb-5">
                        <img src={ blogsmain.length>0&& blogsmain[blogsmain.length-1].image?blogsmain[blogsmain.length-1].image:""} alt="header section image" className="img-fluid img " />
                    </div>

                    <div className="col-lg-7 col-md-6 col-sm-12 mission mt-5 ">
                    <p className="nunito-semi-18-233 pl-3" style={{color:switch1==true?"white":""}}>By {blogsmain.length>0&& blogsmain[blogsmain.length-1].author&&blogsmain[blogsmain.length-1].author.name?blogsmain[blogsmain.length-1].author.name:""} -{moment(blogsmain.length>0&& blogsmain[blogsmain.length-1].created_at?blogsmain[blogsmain.length-1].created_at:"").format("DD MMMM YY")}</p>

                        <h1 className="nunito-sem-36 pl-3" style={{color:switch1==true?"white":""}}>{blogsmain.length>0&& blogsmain[blogsmain.length-1].name?blogsmain[blogsmain.length-1].name:""}</h1>
                        

                        <p className="nunito-reg-14 pl-3 asdjfoapsi390-423" style={{color:switch1==true?"white":""}} dangerouslySetInnerHTML={{ __html: blogsmain.length>0&& blogsmain[blogsmain.length-1].content?blogsmain[blogsmain.length-1].content:"" }}   >
                     </p>
                     
                    <p className="pl-3 nunito-bold-18" style={{color:switch1==true?"white":""}}>
                    {getTime( blogsmain.length>0&& blogsmain[blogsmain.length-1].content?blogsmain[blogsmain.length-1].content:"").text}
            ...
            <div className="mt-5 mb-5 container">
                ...
                <h1 className="roboto-main-head-cst-asdasd text-center" style={{color:switch1==true?"white":""}}>
                    Latest B
                <span>logs</span>
                </h1>
                   <div className="asdjwei4930234-sadfds34 mb-5" style={{backgroundColor:switch1==true?"white":""}}></div>
                ...
                <div className="row mt-5" style={{justifyContent:'center'}}>
                {
                        ...
                        <CardDSP1 switch1={switch1} x={x} image={x.image} head={x.name} title={`by ${x.author&&x.author.name} - ${moment(x.created_at).format("DD MMM yyyy")}`} post={getTime(x.content).text} />
                    </div>)
                    }
                {/* {
